...
export default function DetailPO({
  route,
}: NativeStackScreenProps<AppNavigationType, 'DetailPO'>) {
  ...
  const setujuiPO = async () => {
    await patchPOStatus(data.no_spb, data.no_po, StatusPO.approved).then(
      response => {
  ...
  const rejectPO = async () => {
    await patchPOStatus(data.no_spb, data.no_po, StatusPO.rejected).then(
      response => {
        if (response != undefined) {
          showAlertBS({
            dismissible: false,
  ...
  const cancelPO = async () => {
    await patchPOStatus(data.no_spb, data.no_po, StatusPO.cancel).then(
      response => {
        if (response != undefined) {
          showAlertBS({
            dismissible: false,
    ...
    return (
      <>
        ...
        <View
          ...
          <StatusButton status={data.po_status} />
        </View>
        ...
        <View style={[_s.padding]}>
          <Stack spacing={16}>
            {data.created_by &&
              [StatusPO.approved, StatusPO.rejected, StatusPO.cancel].includes(
                data.po_status,
              ) && (
                <Alert
                  variant="info"
            ...
            {data.last_updated && (
              <Alert
                variant="info"
                type="outlined"
                description={t('last_updated_desc', {
                  date: momentWita(data.last_updated).format(
                    'Do MMMM YYYY, HH:mm',
                  ),
                })}
              />
